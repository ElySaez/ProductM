...
class ProductManager {
    ...
    addProduct(title, description, price, thumbnail, code, stock) {
        ...
        //campos vacios
        //obtiene el nombre del producto y que campo tiene vacio
        for (const key in product) {
            if (Object.hasOwnProperty.call(product, key)) {
                const element = product[key];
                if (!element) {
                    // console.log("El producto: ", `${product.title}` + " no cuenta con: " + `${key}:${element}`);
                    console.log("El producto: ", `${title}` + " no cuenta con: " + `${key}`);
                    return;
                }
            }
        }
